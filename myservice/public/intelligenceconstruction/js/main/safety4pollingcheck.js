 ...
 */
 ...
 (function(){
    ...
    function renderPollingCheckInfo(rootElem, data){
        ...
        if (typeof data == 'object' && data instanceof Array && data.length > 0 ){
            for(let i = 0; i < data.length; i++){
                str += '<div class="row">';
                str += '<div class="info-img"><img src="' + data[i]['imageUrl'] + '"></div>';
                str += '<div class="info-msg"><div class="info-content">' + data[i]['questionDescription'] + '</div>';
                str += '<div class="info-tilte"><span>' + data[i]['checkUser'] + '</span> <span>' + data[i]['createTime'] + '</span></div></div></div>';
            }
        }else{
    ...
    $$.moduleSafetyPollingCheck = function(){
        ...
        function pollingcheckHandler(data){
            let questionInfos = data.data[0].questionInfos;         //获取所有巡检问题
            console.log(questionInfos);
            renderPollingCheckInfo($('.safetydev-pollingcheck-problem-info .panel-body'), questionInfos);
        }

        $$.ajax($$.baseUrl, $$.moduleUrls.safety4pollingcheck, handler);
        $$.ajax($$.baseUrl, $$.moduleUrls.getSafeInfo, pollingcheckHandler);
    }
