...
(function () {
    ...
    $$.moduleFireProtection = function () {
        function handler(data) {
            ...
            //消防设施
            generateFireFightingDeviceChart(mychart, data.data.fireFightingDevice.fireFightingDeviceManage);
            renderFireFightingDeviceTableInfo($('.fire-fighting-device-manage-table table tbody'), data.data.fireFightingDevice.fireFightingDeviceExtinguisher);
            //重点部位
            generateKeyPositionDeviceChart(mychart1, data.data.keyPosition.keyPositionManage);
            renderKeyPositionTableInfo($('.key-position .col-sm-4 tbody'), data.data.keyPosition.info);
            //安全疏散
            generateSafetyEvacuateChart(mychart2, data.data.safetyEvacuate.safetyEvacuateManage);
        }
        $$.ajax($$.baseUrl, $$.moduleUrls.safety4fireprotection, handler);
    }
})()